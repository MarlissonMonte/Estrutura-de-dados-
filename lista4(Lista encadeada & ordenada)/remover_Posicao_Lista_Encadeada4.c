...
bool removerDaPos(int posRemocao, LISTA *l){
    if(vazia(l)) // nao tem o que remover
        return false;
    NO* pAnterior = l ->cabeca;
     // se a posicao que eu queira remover for a 0
    if(posRemocao == 0){
    ...
    // demais casos(meio ou fim da lista)
        int posAtual = 1;
        while((pAnterior -> prox)&&(posAtual < posRemocao)){
            pAnterior = pAnterior -> prox;
            posAtual ++;
        }
    // se chegar no item que eu quero remover, remova-o
           if((pAnterior -> prox)&& (posAtual == posRemocao)){  
              NO* pApagar = pAnterior ->prox;
              pAnterior ->prox= pApagar ->prox;
              free(pApagar);
              return true;
     }
