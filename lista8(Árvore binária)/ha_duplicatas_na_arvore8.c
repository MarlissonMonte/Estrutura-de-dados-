...
// Insere todas as chaves da arvore no vetor de chaves
// A funcao percorrerArvore recebe um nO de uma arvore binaria de busca,
// um vetor de chaves e um inteiro que representa o numero de elementos no vetor.
// Ela percorre a arvore de forma recursiva e, a cada nO, insere sua chave no vetor de chaves.
void percorrerArvore(ARVORE_BINARIA *no, TIPOCHAVE *chaves, int *n)
{
    if (no != NULL)
    {
        percorrerArvore(no->esq, chaves, n); // Chama a fun��o recursivamente para o filho esquerdo
        chaves[(*n)++] = no->item.chave;     // Insere a chave do n� atual no vetor de chaves
        percorrerArvore(no->dir, chaves, n); // Chama a fun��o recursivamente para o filho direito
    }
}

// Verifica se h� algum valor duplicado no vetor de chaves
// A fun��o verificarDuplicados recebe um vetor de chaves e o n�mero de elementos no vetor.
// Ela verifica se h� algum valor duplicado no vetor, comparando todos os elementos entre si.
bool verificarDuplicados(TIPOCHAVE *chaves, int n)
{
    ...
    return false; // Retorna falso se n�o houver valores duplicados
}
...
TIPOCHAVE chaves[100];               // Cria um vetor de chaves com tamanho m�ximo de 100
    int n = 0;                           // Inicializa o n�mero de elementos no vetor como 0
    percorrerArvore(arvore, chaves, &n); // Preenche o vetor de chaves com as chaves da �rvore

    return verificarDuplicados(chaves, n); // Retorna verdadeiro se houver valores duplicados e falso caso contr�rio
